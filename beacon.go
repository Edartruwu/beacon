...
// Searcher combines multiple search techniques for optimal results
type Searcher[T Searchable] struct {
	items         []T
...
// NewSearcher creates a new searcher with enhanced indexing
func NewSearcher[T Searchable](items []T, minSimilarity float64, debugMode bool) *Searcher[T] {
...
func (is *Searcher[T]) Search(req SearchRequest[T]) (interface{}, error) {
...
func (is *Searcher[T]) calculateScore(
	idx *SearchIndex[T],
	queryLower string,
	queryWords []string,
...
func (is *Searcher[T]) filterItems(filter Filter) []SearchIndex[T] {
	if filter == nil {
...
func (is *Searcher[T]) GetItemCount() int {
	return len(is.items)
}

// GetMinSimilarity returns the minimum similarity threshold
func (is *Searcher[T]) GetMinSimilarity() float64 {
	return is.minSimilarity
}

// SetMinSimilarity updates the minimum similarity threshold
func (is *Searcher[T]) SetMinSimilarity(minSimilarity float64) {
	is.minSimilarity = minSimilarity
}

// SetDebugMode enables or disables debug mode
func (is *Searcher[T]) SetDebugMode(debug bool) {
	is.debugMode = debug
}
